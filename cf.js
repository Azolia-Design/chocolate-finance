const mainScript = () => {
    ...
    function createPartnerHTML(template, partner) {
        ...
        html.find('[data-partner="img"]').attr('src',partner.image.url);
    ...
    function updateInterestRate(wrapper) {
        ...
        // let fixedRate = $('[data-rate-source="fixed"]').text();
    ...
    SCRIPT.homeScript = () => {
        ...
        function homeChartHandle() {
            ...
            const limitAmount = parseInt($('[data-rate-source="amount"]').text().replace(',','')) * 1000;
            ...
            const fixedRate = parseFloat($('[data-rate-source="fixed"]').text()) / 100;
        ...
        function homeGetFaq() {
            getAllDataByType('faq').then((res) => {
                if (res) {
                    let activeFaqItem = res.filter(i => i.data.display_on[0]?.display_on_cholate_finance)
                    let allFaq = sortAsc(activeFaqItem, true, 'order_on_homepage', true)
                    $('.home-faq-main').html('')
                    allFaq.forEach((i) => {
                        if (i.data.display_on[0]?.display_on_cholate_finance) {
                            if (i.data.cf_config[0]?.show_on_homepage) {
    ...
    SCRIPT.howItWorksScript = () => {
        if ($('.intro-wrap').length) {
            setTimeout(() => $('.intro-wrap').addClass('loaded'), 300);
        }
        if ($(window).width() < 767) {
        ...
        function homeGetFaq() {
            getAllDataByType('faq').then((res) => {
                if (res) {
                    ...
                    allFaq.forEach((i) => {
                        if (i.data.display_on[0]?.display_on_cholate_finance) {
                            if (i.data.cf_config[0]?.show_on_how_it_works) {
                                createFaqNew(i).appendTo($('.home-faq-main'))
                            }
                        }
                    })
            ...
            if ($(window).width() > 991) {
                ...
                $('.how-expect-card').on('click', function(e) {
                    ...
                    if ($(this).hasClass('active')) {
                        ...
                        $('.how-expect-card-wrap').removeClass('active-1 active-2 active-3')
                        $('.how-expect-card-wrap').addClass(`active active-${target+1}`)
                        $('.how-expect-card-wrap').css('--default-height', `${allHeight[target]}px`)
                        $('.how-expect-card .how-expect-card-sub').attr('style', '')
                        $(this).find('.how-expect-card-sub').css('--default-width', `${allWidth[target]}px`)
                        $('.how-expect-card').attr('style', '')
                        $('.how-expect-card').removeClass('active')
                        $('.how-expect-card').addClass('de-active')
                        $(this).addClass('active')
        ...
        function aboutGetFaq() {
            getAllDataByType('faq').then((res) => {
                if (res) {
                    ...
                    allFaq.forEach((i) => {
                        if (i.data.display_on[0]?.display_on_cholate_finance) {
                            if (i.data.cf_config[0]?.show_on_about_us)
                            createFaqNew(i).appendTo($('.home-faq-main'))
                        }
                    })
                    updateInterestRate('.home-faq-main');
    ...
    SCRIPT.blogScript = () => {
        ...
        const currentUpdate = async () => {
            const layoutFunc_Current = (blogs, sectionWrapper) => {
                const investFeaTemplate = sectionWrapper.find('.blog-invest-fea-inner').eq(0).clone();
                const ecoListTemplate = sectionWrapper.find('.blog-eco-item').eq(0).clone();
                sectionWrapper.find('.blog-invest-fea').html('');
                sectionWrapper.find('.blog-eco-list').html('');
                blogs.forEach((e, i) => {
                    if (i == 0) {
                        let investFeaHtml = createHtml(investFeaTemplate, e, true);
                        sectionWrapper.find('.blog-invest-fea').append(investFeaHtml);
                    } else {
                        let ecoListHtml = createHtml(ecoListTemplate, e, true);
                        sectionWrapper.find('.blog-eco-list').append(ecoListHtml)
                    }
                })
                // investSwiperInteraction(sectionWrapper)
            }
            const layoutFunc_CurrentHero = (blogs) => {
                const heroThumbTemplate = $('.swiper-slide.blog-hero-item').eq(0).clone();
                const heroPagiTemplate = $('.blog-nav-item').eq(0).clone();
                $('.swiper-wrapper.blog-hero-wrapper').html('');
                $('.blog-hero-nav-inner').html('');
                blogs.forEach((i) => {
                    let heroThumbHtml = createHtml(heroThumbTemplate, i, true);
                    $('.swiper-wrapper.blog-hero-wrapper').append(heroThumbHtml)
                    let heroPagiHtml = createHtml(heroPagiTemplate, i, true);
                    $('.blog-hero-nav-inner').append(heroPagiHtml)
                })
                blogHeroInteraction();
            }
            const allBlog = await getAllDataByType('blog').then((res) => {
                let heroBlogs = res.filter(blog => blog.data.isfeatured == true).slice(0,4);
                layoutFunc_CurrentHero(heroBlogs)
                layoutFunc_Current(res, $('.sc-blog-wrap.all-list'))
                $('[data-blog-dou="author-link"]').css('pointer-events', 'none')
                if ($('.intro-wrap').length) {
                    $('.intro-wrap').addClass('loaded');
                }
            });
    ...
    SCRIPT.articleScript = () => {
        ...
        function updateContent(res) {
            updateUrl(res.uid,'article',res.data.leading_paragraph[0].text, res.data.thumbnail.url);
        ...
        function updateRelBlog(blog) {
            ...
            // Rel by all, except current Blog
            let param = window.location.pathname;
            let currId = param.replace('/article/', '')
            getAllDataByType('blog', pageSize=4).then((res) => {
                let list = res.filter(i => i.uid != currId)
                const relListTemplate = $('.sc-art-rel .blog-eco-item').eq(0).clone();
                $('.sc-art-rel .blog-eco-list').html('');
                list.forEach((i, idx) => {
                    if (idx >= 4) return;
    ...
    SCRIPT.faqsScript = () => {
        ...
        function updateUICate(allFaqCate) {
            ...
            allFaqCate.forEach((faqCate, i) => {
                if (faqCate.data.display_on[0]?.display_on_chocolate_finance) {
                    //Tab
    ...
    SCRIPT.contactUsScript = () => {
        ...
        function updateParam() {
            ...
            if ($('.intro-wrap').length) {
                setTimeout(() => $('.intro-wrap').addClass('loaded'), 1);
            }
        }
        // updateParam();
